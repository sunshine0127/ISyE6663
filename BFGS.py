...
while sum(deri_f_1(x_temp)**2)>= 1e-6*(t+1):
    ...
    alpha_temp=s
    while sum(deri_f_1(x_temp+alpha_temp*d_temp)*d_temp) > 0.1*sum(deri_f_1(x_temp)*d_temp):
    ...
    H_temp=H_temp+(1+q_temp/p_temp)*p.T*p/p_temp-p.T*q*H_temp/q_temp-H_temp*q.T*p/q_temp
    sum1=sum1+1
...
while sum(deri_f_2(x_temp)**2)>= 1e-6*(t+1):
    ...
    alpha_temp=s
    while sum(deri_f_2(x_temp+alpha_temp*d_temp)*d_temp) > 0.1*sum(deri_f_2(x_temp)*d_temp):
        alpha_temp=b*alpha_temp
